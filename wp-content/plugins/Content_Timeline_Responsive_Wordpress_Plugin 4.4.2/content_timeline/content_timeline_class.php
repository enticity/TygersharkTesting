		...
		foreach ( $query->posts as $match) {
			if($searchVal != ''){
				if(strpos(strtolower($match->post_name), $searchVal) !== false){
					...
					echo '<li><img style="margin-right:5px; margin-top:4.5px;" src="'.$this->url.'timthumb/timthumb.php?src='.$thumbn[0].'&w=150&h=150" width="32" height="32" alt="" /><img id="TBct_timelineSubmitLoader" class="alignright" src="\'+pluginUrl+\'/images/ajax-loader.gif" /><span class="timelinePostCompleteName">'.$match->post_title .'</span><a href="'.$match->ID.'">Add Post</a><span class="clear"></span></li>';
				}
			}
		}
		die();
